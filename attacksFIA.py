...
tf.flags.DEFINE_string('output_dir', './adv/NAA_inc3/', 'Output directory with images.')
...
"""parameter for NAA"""
tf.flags.DEFINE_integer('ens', 30, 'Aggregated N for NAA or Mask number for FIA.')
"""parameter for FIA"""
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        if 'FDA' in FLAGS.attack_method:
            loss = get_fda_loss(opt_operations)  # enhance smaller half features, suppress bigger half features
        elif 'NRDM' in FLAGS.attack_method:
            loss = get_nrdm_loss(opt_operations)  # maximize feature distance
        elif 'FIA' in FLAGS.attack_method:
            if 'FIACE' == FLAGS.attack_method:
                weight_loss = -tf.losses.softmax_cross_entropy(label_ph[:FLAGS.batch_size], logits[:FLAGS.batch_size])
            elif 'FIAP' == FLAGS.attack_method:
                weight_loss = tf.reduce_sum(problity * label_ph)
            else: # 'FIAZ'
                weight_loss = tf.reduce_sum(logits * label_ph)
            weights_tensor = tf.gradients(weight_loss, opt_operations[0])[0]  # [2*batch_size,...].
            loss = -get_fia_loss(opt_operations, weights_ph)
        elif 'NAA' in FLAGS.attack_method:
            if 'NAACE' == FLAGS.attack_method:
                weight_loss = -tf.losses.softmax_cross_entropy(label_ph[:FLAGS.batch_size], logits[:FLAGS.batch_size])
            elif 'NAAP' == FLAGS.attack_method:
                weight_loss = tf.reduce_sum(problity * label_ph)
            else: # 'NAAZ'
                weight_loss = tf.reduce_sum(logits * label_ph)
            weights_tensor = tf.gradients(weight_loss, opt_operations[0])[0]
            loss = -get_NAA_loss(opt_operations, weights_ph, base_feature)
        elif 'MIM' in FLAGS.attack_method:
            if 'MIMCE' == FLAGS.attack_method:  # max ce loss
                loss = tf.losses.softmax_cross_entropy(label_ph[:FLAGS.batch_size], logits[FLAGS.batch_size:])
            elif 'MIMP' == FLAGS.attack_method:  # max neg prob. loss may be zero! so add stablisher to gradient!
                loss = -tf.reduce_sum(label_ph[FLAGS.batch_size:]*problity[FLAGS.batch_size:])
            else:  # 'MIMZ', max neg logits
                loss = -tf.reduce_sum(label_ph[FLAGS.batch_size:]*logits[FLAGS.batch_size:])
        else:
            loss = entropy_loss  # max

        gradient = tf.gradients(loss, adv_input)[0]

        noise = gradient  # enhancing gradient
        ...
        noise = noise / (tf.reduce_mean(tf.abs(noise), [1, 2, 3], keep_dims=True) + 0 * 1e-5)
        ...
        if 'PI' in FLAGS.attack_method:
            ...
            adv_input_update = adv_input_update + alpha * tf.sign(noise)  # this code maximize loss

        saver = tf.train.Saver()
        with tf.Session() as sess:
            saver.restore(sess, checkpoint_path)
            count = 0
            for images, names, labels in tqdm.tqdm(utils.load_image(FLAGS.input_dir, FLAGS.image_size, FLAGS.batch_size)):
                ...
                for i in range(num_iter):
                    if i == 0:
                        ...
                        feature_base = sess.run([opt_operations[0]],
                                                feed_dict={ori_input: images_base, adv_input: images_base,
                                                           label_ph: labels})
                        feature_base = feature_base[0][:FLAGS.batch_size]
                        if 'FIA' in FLAGS.attack_method:
                            for l in range(FLAGS.ens):
                                w, = sess.run([weights_tensor], feed_dict={ori_input: images, adv_input: images, label_ph: labels})
                ...
                # assert not np.any(np.isnan(images_adv))
                utils.save_image(images_adv, names, FLAGS.output_dir)
        print(f'save {FLAGS.output_dir}')
